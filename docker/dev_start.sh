...
function local_volumes() {
  ...
  volumes="${volumes} -v ${LOCAL_DIR}:/YardPlanning_root"
...
eval docker create -it \
      ...
      --ulimit core=-1 \
      --dns=114.114.114.114 \
      --add-host YardPlanning_docker:127.0.0.1 \
      --add-host ${LOCAL_HOST}:127.0.0.1 \
      --hostname in_YardPlanning_docker \
...
echo "docker started"
# check_port
docker exec ${DOCKER_NAME} /bin/bash -c "ln -s -f /YardPlanning_root/YardPlanning /"
docker exec ${DOCKER_NAME} chown ${USER_ID}:${GRP_ID} /YardPlanning
